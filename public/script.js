...
form.addEventListener('submit',async(event)=>{event.preventDefault();clearMessage();try{validateUserId();const weight=validateWeight();if(!stepsInput.value){await loadActivity();}const steps=Number(stepsInput.value);const pulse=validatePulse(pulseInput.value);const response=await fetch('/api/fitness/calories',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({steps,pulse,weight})});const data=await response.json();if(!response.ok)throw new Error(data.message||'Ошибка расчёта калорий');const note=data.doubled?'Коэффициент был удвоен, так как пульс превышает 160.':'Использован базовый коэффициент 0.04.';renderMessage('success',`<div class="result-main">Сожжено калорий: ${data.calories}</div><div class="result-note">${note}</div>`);}catch(error){renderMessage('error',error.message);}});